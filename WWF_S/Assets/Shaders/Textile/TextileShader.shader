Shader "TextileShader" {
	...
	SubShader {
        ...
        #include "Assets/Shaders/ShaderIncludeLib/Noise.hlsl"
		...
		Pass {
            ...
            //TEXTURE2D(_DamageMap);
			//SAMPLER(sampler_DamageMap);
			...
			Varyings LitPassVertex(Attributes IN) {
                ...
                float4 windDirectionOS = mul(unity_WorldToObject , _WindDirection);

                float dmg = tex2Dlod(_DamageMap, float4(IN.uv.xy, 0, 0)).r;

                float f = dot(IN.normalOS.rgb, windDirectionOS.rgb);
                if (f < 0.1) f = 0;

                f *= (1 - IN.color.r);

                float3 forward = mul((float3x3)unity_CameraToWorld, float3(0,0,1));
                float3 forwardOS = mul(unity_WorldToObject , forward);
                float3 downOS = mul(unity_WorldToObject , float3(0, -1, 0));

                float n = (noise(float3(IN.positionOS.rgb * 4) + _Time.r * 8));

                float3 deformation =  windDirectionOS * _DeformationScale * f * _WindSpeed * n * (1 + dmg);

                float3 sagDir = (downOS + IN.normalOS * 0.1);
                deformation += sagDir * dmg * _DmgSagScale;
                //float3 deformation = float3(0,0, dmg*3);

                // float f = dot(OUT.normalWS, float3(1, 0, 0));
                // if (f < 0) f = 0;
                // float3 deformation =  float3(1, 0, 0) * _DeformationScale * f;

				VertexPositionInputs positionInputs = GetVertexPositionInputs(IN.positionOS.xyz + deformation);
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
                //half4 damageMap = SAMPLE_TEXTURE2D(_DamageMap, sampler_DamageMap, IN.uv);
                half4 damageMap = tex2Dlod(_DamageMap, float4(IN.uv.xy, 0, 0)).r;;
                if (damageMap.r > 0.8)
                    discard;

				half4 baseMap = lerp(float4(1,1,1,1), SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv), _TextureInfluence);
                ...
                // float d = Linear01Depth(IN.depth);
