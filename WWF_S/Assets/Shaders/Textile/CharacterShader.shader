Shader "CharacterShader" {
	Properties {
        ...
        _WindDirection ("Wind Direction", Color) = (1, 0, 0, 0)
	...
	SubShader {
		...
		Pass {
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
                ...
                //fragmentEyeDepth /= 100;
                //return float4(fragmentEyeDepth,fragmentEyeDepth,fragmentEyeDepth, 1);
                
                float rawDepth = SampleSceneDepth(IN.screenPos.xy / IN.screenPos.w);
                float sceneEyeDepth = LinearEyeDepth(rawDepth, _ZBufferParams);

                float d = sceneEyeDepth - fragmentEyeDepth;

                //return float4(d,d,d, 1);
                if (d > -0.001 && d < 0.05)
                    discard;
                // if (sceneEyeDepth < 0.5)
                //     discard;

                //return float4(sceneEyeDepth,sceneEyeDepth,sceneEyeDepth, 1);
