...
Shader "Custom/DMETerrainShader" {
	Properties {
	...
	SubShader {
		...
		Pass {
		 ...
		 //
			...
			// 	
			// 	float colorStep =  round((IN.color.r - 0.01f) * 3) / 3;
			// 	
			...
			// 	float3 colorStep2 = round((IN.color.g  - 0.01f) * 3) / 3;
			// 	half3 colorCel2 = lerp(color, _Color3.rgb, colorStep2);
			// 	color = lerp(colorSmooth2, colorCel2, 0.5f);
			//
			// 	// Grass stuff
			// 	if (IN.isGrass) {
